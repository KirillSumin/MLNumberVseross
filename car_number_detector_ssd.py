...
paths = 'C:/Users/IlyaP/Desktop/Python/ML/Car-number-plate-detection-and-recognition-master/model1_nomer.tflite'
...
def ocr_tesseract(image):
    ...
    #                                                        '--oem 0'
    #                                                        '-c tessedit_char_whitelist=0123456789Р™Р¦РЈРљР•РќР“РЁР©Р—РҐРЄР¤Р«Р’РђРџР РћР›Р”Р–Р­РЇР§РЎРњРРўР¬Р‘Р®РЃ'
    ...
    with detection_graph.as_default():
        with tf.compat.v1.Session(graph=detection_graph) as sess:
            ...
            # Video mode
                ...
                CAMERA_IP_PORT = 'http://192.168.0.191:4747/video'
                cap = cv2.VideoCapture(CAMERA_IP_PORT)
                # if not cap.isOpened():
                ...
                while True:
                    ...
                    numbers = cut_detect_numbers(scores, boxes, image_np)
                    if len(numbers) != 0:
                        elements = [i for i in numbers[0][0]]
                        print(elements)
                        if elements[1].isalpha():
                            elements = elements[1:]
                        if elements[5].isalpha():
                            elements = elements[:6]
                            data.append(''.join(elements))

                    if len(data) == 20:
                        uniques = set(data)
                        freqs = [[item, data.count(item)] for item in uniques]
...
def main():
    ...
    # -----------------------------------------------------------------------------------------------
    # Apply detector
    detector(detection_graph, category_index, args[0], args[1])

def launch(mode, image_path):
    global args
    args = [mode, image_path]
    main()

if __name__ == '__main__':

    # launch('1', '8402943.jpg')
    ...
    args = [args.mode, args.image_path]
